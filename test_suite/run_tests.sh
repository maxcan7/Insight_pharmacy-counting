...
GRADER_ROOT=$(readlink -f $(dirname ${BASH_SOURCE}))
...
function check_project_struct {
  find_file_or_dir_in_project ${PROJECT_PATH} pharmacy_counting.sh
...
function setup_testing_input_output {
  ...
  cp -r ${PROJECT_PATH}/pharmacy_counting.sh ${TEST_OUTPUT_PATH}
